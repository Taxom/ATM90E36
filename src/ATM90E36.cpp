...
double ATM90E36::GetImportEnergy() {
  unsigned short ienergyT = CommEnergyIC(READ, ANenergyT, 0xFFFF);
  ...
  return (double)ienergyT  / 3200; //returns kWh
}
//Indi Energy Channels 
double ATM90E36::GetImportAenergy(){
	unsigned short ienergyA = CommEnergyIC(READ, ANenergyA, 0xFFFF);	
return (double)ienergyA / 3200; //returns kWh
}

double ATM90E36::GetImportBenergy(){
	unsigned short ienergyB = CommEnergyIC(READ, ANenergyB, 0xFFFF);	
return (double)ienergyB / 3200; //returns kWh
}

double ATM90E36::GetImportCenergy(){
	unsigned short ienergyC = CommEnergyIC(READ, ANenergyC, 0xFFFF);	
return (double)ienergyC / 3200; //returns kWh
}

double ATM90E36::GetExportAenergy(){
	unsigned short eenergyA = CommEnergyIC(READ, APenergyA, 0xFFFF);	
return (double)eenergyA / 3200; //returns kWh
}

double ATM90E36::GetExportBenergy(){
	unsigned short eenergyB = CommEnergyIC(READ, APenergyB, 0xFFFF);	
return (double)eenergyB / 3200; //returns kWh
}

double ATM90E36::GetExportCenergy(){
	unsigned short eenergyC = CommEnergyIC(READ, APenergyC, 0xFFFF);	
return (double)eenergyC / 3200; //returns kWh
}


double ATM90E36::GetImportVarh(){
	unsigned short renergyT = CommEnergyIC(READ, RNenergyT, 0xFFFF);
	
return (double)renergyT / 3200; //returns kWh
}

double ATM90E36::GetExportVarh(){
	unsigned short renergyT = CommEnergyIC(READ, RPenergyT, 0xFFFF);
...
double ATM90E36::GetExportEnergy() {
  ...
  unsigned short eenergyT = CommEnergyIC(READ, APenergyT, 0xFFFF);
  ...
  return (double)eenergyT  / 3200; //returns kWh 
}
...
{  
  ...
  CommEnergyIC(WRITE, GainA, 0x01E5);       // Line calibration gain
  CommEnergyIC(WRITE, PhiA, 0x0000);        // Line calibration angle
  CommEnergyIC(WRITE, GainB, 0x02BC);       // Line calibration gain
  CommEnergyIC(WRITE, PhiB, 0x0000);        // Line calibration angle
  CommEnergyIC(WRITE, GainC, 0x0258);       // Line calibration gain
  CommEnergyIC(WRITE, PhiC, 0x0000);        // Line calibration angle
  CommEnergyIC(WRITE, CSOne, 0x00FE);       // Checksum 1
  ...
  CommEnergyIC(WRITE, UgainA, 0xAFA5);      // A SVoltage rms gain
  CommEnergyIC(WRITE, IgainA, 0x1036);      // A line current gain
  CommEnergyIC(WRITE, UoffsetA, 0x0000);    // A Voltage offset
  CommEnergyIC(WRITE, IoffsetA, 0x0000);    // A line current offset
  CommEnergyIC(WRITE, UgainB, 0xAF3C);      // B Voltage rms gain
  CommEnergyIC(WRITE, IgainB, 0x102B);      // B line current gain
  CommEnergyIC(WRITE, UoffsetB, 0x0000);    // B Voltage offset
  CommEnergyIC(WRITE, IoffsetB, 0x0000);    // B line current offset
  CommEnergyIC(WRITE, UgainC, 0xAEE0);      // C Voltage rms gain
  CommEnergyIC(WRITE, IgainC, 0x1038);      // C line current gain
  CommEnergyIC(WRITE, UoffsetC, 0x0000);    // C Voltage offset
  CommEnergyIC(WRITE, IoffsetC, 0x0000);    // C line current offset
  CommEnergyIC(WRITE, IgainN, 0x0000);      // C line current gain
  CommEnergyIC(WRITE, CSThree, 0xE296);     // Checksum 3
